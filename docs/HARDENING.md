...
| **Round 11** — real **KernScores** piano (Chopin mazurkas, Mozart sonatas) via oracle | 8 | 0/8 exact — **found G18**: multi-staff kern `*^` column-shift drops ~half the notes on piano scores (2 spines + splits). Confirmed pitch-level (crisp_notation 46-89% of music21). Top priority for the next kern-reader pass. |
...
| **Rounds 17-19** — `--quorum` (music21 + Verovio) over 42 fresh files: Bach 4-part chorales (kern + music21 MusicXML), music-encoding structure/lyrics MEI, Palestrina masses (kern), Monteverdi madrigals (mxl) | 42 | **0 consensus-bugs.** All confirmed correct (both-agree) or resolved-correct (Verovio over-expanded repeats). crisp_notation validated across Bach chorales, Renaissance modal polyphony (musica ficta), and multi-section/verse MEI. Only oracle-side SKIPs (Verovio couldn't parse some Monteverdi mxl). |
| **Round 20** — Bach chorale MusicXML + essen-folksong ABC via `--quorum` (now with the **abc2midi tiebreaker**) | 27 | Bach MusicXML all correct. ABC flagged 1 "consensus-bug" (`essentune11`) — the same no-carry accidental false-positive; **abc2midi (reference) confirms crisp_notation** (98.4% vs the oracles' 92%), so the quorum now **auto-resolves it to CHECK✓**. The quorum consults abc2midi on ABC consensus-bugs, since music21 & Verovio share the no-carry convention. |
| **Round 21** — exotic: **Schoenberg Op.19 (atonal)** + Beethoven string quartets (music21 corpus) via `--quorum` | 6 | **0 consensus-bugs.** Atonal Schoenberg (dense accidentals, no key) → resolved-correct; Beethoven quartets suspect only from Verovio repeat-expansion (crisp_notation ≈ music21, off by 1 on op18no1-1). |
...
* `test/oracle_validated_test.dart` — committed, self-contained: pins the
  spec-correct ABC behaviours the oracle campaign proved crisp_notation reads right
...
* `tool/roundtrip_sweep.dart` — diagnostic over a local corpus of real scores
  (not part of this repo); compares the note multiset
...
across **all** parts (crisp_notation via the multi-part `staffSystemFrom*`, to line
...
investigated, not blindly blamed on crisp_notation.
...
score). crisp_notation's importer is, on this corpus, essentially correct.

**Two rounds of "divergences" turned out to be bugs in the oracle *tool*, not in
crisp_notation** — a live demonstration of the user's caution that the oracle script
...
2. It then counted only voice 1 (`measure.elements`), ignoring the model's
   ...
   "crisp_notation dropped notes." Fixed to read all four voices. This collapsed the
...
* **Verovio** (`tool/verovio_dump.py`, `dart run tool/oracle_diff.dart --oracle
  ...
  **humlib** (authoritative `**kern`), and parses ABC (broken rhythm, keys) and
  ...
  tune where music21 was wrong about K:D, **crisp_notation agrees with Verovio 100%**,
  confirming crisp_notation (not music21) was right. Caveat: Verovio's note times come
...
* **Ensemble quorum** (`--quorum`) — the actual answer. Compare crisp_notation against
  **both** music21 and Verovio and classify each score:
  - **OK** — both oracles agree with crisp_notation (high-confidence correct).
  - **consensus-bug** — the two independent oracles agree with *each other* but
    not with crisp_notation → a genuine crisp_notation-bug signal.
  - **oracle-split** — the oracles disagree with each other → an oracle
    ...
    against crisp_notation.

  Across every score tried, the quorum finds **0 consensus-bugs**: there is no
  case where two independent parsers agree against crisp_notation. Every divergence
...
(both music21 **and** Verovio read F♯ where crisp_notation reads F♮ on 4 notes). But
...
to the end of the bar** — so crisp_notation (F♮) is *spec-correct*, and *both* oracles
share a common non-spec no-carry convention. A minimal test confirms it:
`K:G` … `=f f | f f` → crisp_notation `F♮ F♮ | F♯ F♯` (carry within bar, reset at the
...
gives `F♮ F♮ | F♯ F♯`, **identical to crisp_notation** — so crisp_notation and the
reference implementation agree, and music21 + Verovio share a non-spec no-carry
bug. Across 45 real ABC tunes, crisp_notation's **pitch set matches abc2midi on
37/45** (only 11 pitch-classes differ total) — the reference engine validates
crisp_notation's ABC reading. (abc2midi's overall midi@ql match is low only because
...
**abc2midi's own tie-parsing bugs**, not crisp_notation errors: it errors on the
...
spec, crisp_notation is correct on all of them. The newer **abc2midiu v1.0**
...
repeat expansion, tie parsing — while crisp_notation reads them spec-faithfully.**
...
be wrong and crisp_notation right. Consensus-bugs still must be checked against the
...
| G3 | high | musicxml reader | A slur `start`/`stop` imbalance (a `type="continue"` reusing a number, or a lost `stop`) left a slur open → parse aborted *"Unclosed `<slur>`"*. | `crisp_notation info Debussy_Mandoline.xml` | **fixed** — dangling slur dropped, parse continues |
| G4 | high (crash) | layout engine | A degenerate `Hairpin(eN → eN)` (start == end) threw `must run forward in reading order` — uncaught. | `crisp_notation render Dichterliebe01.xml …` | **fixed** (a2… ) |
| G5 | high (crash) | layout engine | A `Pedal(e0 → e29)` whose end id is not in the imported score threw `references an unknown note element id` — uncaught. | `crisp_notation render OSMD_Function_Test_Pedals.musicxml …` | **fixed** |
| G9 | high (crash) | musicxml reader | A **guitar-tablature** MusicXML staff carries `<clef><sign>TAB</sign>`, which `_clefOf` didn't recognize → threw `Unsupported clef: TAB5`, aborting the whole import. | `crisp_notation render BrookeWestSample.mxl …` | **fixed at the source** — `TAB` maps to the guitar clef (`treble8vb`, sounding 8vb) so the staff's real `<pitch>`es render; any other/malformed sign now defaults to treble instead of aborting (reader-leniency, per G3/G6/G7). BrookeWest → 2 staves. Regression test in `musicxml_test.dart`. |
| G10 | high (crash) | CLI file read | A **UTF-16 LE (BOM)** MusicXML — a legal, common export encoding, its XML prolog even declaring `encoding="UTF-16"` — threw `FileSystemException: Failed to decode data using encoding 'utf-8'` because the CLI read every text score via `File.readAsStringSync` (UTF-8 only). | `crisp_notation render test_UTF16LEBOM_decoding_nested_tuplet.musicxml …` | **fixed at the byte→String boundary** — a `_readText` helper sniffs the BOM (UTF-16 LE/BE, UTF-8) and decodes accordingly (`dart:convert` ships no UTF-16 codec); all text-format reads route through it. CLI round-trip test in `cli_test.dart`. |
...
| G31 | high (engraving) | **staff-system line breaking** | The greedy system packer estimated line width from measure widths, but a later justified/aligned staff-system layout could exceed `maxWidth`; K545 T57 and T70 overflowed the rendered page even though the estimate said they fit. Follow-ups exposed two inverse regressions: one-measure continuation systems were either stretched unnaturally (T70) or left with short staff lines (T43), and imported MusicXML source breaks were preserved too literally, stranding short systems even when the following measures fit. | K545 SVG/PNG at `--width 120`, bars 43, 47, 57 and 70 | **fixed** — after rendering a candidate system, `layoutStaffSystemSystems` backs off measures until the actual laid-out width fits (unless a single measure itself is overwide). Single-measure continuation systems keep natural note spacing while padding the staff and closing system barline to the target width. MusicXML source system breaks are now soft when the current line is badly underfilled; explicit caller `systemBreaks` remain hard. Regressions in `multi_system_test.dart`; verified by K545 r16 visual crop sweep. |
