...
| [`partitura_cli`](packages/partitura_cli) | Command-line tool: inspect scores, convert between MusicXML / MIDI / `.gp` / ABC, render to SVG (notation or tab). Pure Dart. | `partitura_core` |
...
**Interchange.** MusicXML, MIDI, the `.gp3`–`.gp5`/`.gpx`/`.gp` tablature
family (with GPIF), and ABC — all importing and (where applicable) exporting
through the one `Score` model, so any pair round-trips for shared data.
