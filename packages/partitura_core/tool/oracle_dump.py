...
the same file with partitura and compares its note multiset against this one:
a mismatch is far more likely a partitura import bug than a music21 one, and
...
def dump(path):
    ...
    for n in score.recurse().notes:  # Note and Chord objects, all parts
        ...
        if ql == 0:  # grace note — partitura folds these into the main note
            continue
        for p in n.pitches:
