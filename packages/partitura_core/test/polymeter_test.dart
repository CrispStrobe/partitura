...
void main() {
  ...
  group('wrapped polymetric document', () {
    ...
    StaffSystem longPolymeter() {
      ...
      return StaffSystem([
        Score.simple(
            ...
            notes: top),
        Score.simple(
            clef: Clef.bass, timeSignature: const TimeSignature(6, 8), notes: bot),
      ]);
    }

    test('the first system shows each staff its own meter', () {
      final wrapped = layoutStaffSystemSystems(longPolymeter(), settings,
          maxWidth: 45);
    ...
    test('a per-staff meter change at a system start is restated', () {
      ...
      final atChange =
          wrapped.systems.firstWhere((s) => s.firstMeasure == 2);
      // The lower staff restates its new 3/4 at the boundary...
      expect(timeSigGlyphs(atChange.layout.staves[1]), ['timeSig3', 'timeSig4']);
      // ...and the (unchanged) upper staff also restates, keeping them aligned.
      expect(timeSigGlyphs(atChange.layout.staves[0]), ['timeSig3', 'timeSig4']);
    });
  });
}
