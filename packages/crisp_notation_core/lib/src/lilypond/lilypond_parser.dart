...
class LilyPondParser {
  ...
  LyNode? _parseNode() {
    ...
    if (token.kind == TokenKind.command) {
      ...
      int argsCount = 0;
      switch (token.value) {
        case 'new': argsCount = 1; break; // e.g. \new Staff
        case 'with': argsCount = 1; break;
        case 'relative': argsCount = 1; break; // e.g. \relative c'
        case 'time': argsCount = 1; break; // e.g. \time 4/4
        case 'clef': argsCount = 1; break; // e.g. \clef treble
        case 'key': argsCount = 2; break; // e.g. \key c \major
  ...
  LyNode _parseChord() {
    ...
    _parseDurationAndScripts((dur) { duration = dur; }, scripts);
  ...
  LyNode _parseWord(String word) {
    ...
    if (restRe.hasMatch(word)) {
      ...
      final scripts = <String>[];
      _parseDurationAndScripts((dur) { if (duration == null) duration = dur; }, scripts);
  ...
  void _parseDurationAndScripts(void Function(String) setDuration, List<String> scripts) {
